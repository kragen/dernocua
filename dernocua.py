...
class Bundle:
    def __init__(self, dirname):
        ...
        self.output_dir = 'dernocua-' + self.get_version()
        self.cached_titles = {}
    ...
    def get_version(self):
        v = self.relation('version')['dernocua']
        return 'prerelease' if not v else v[0]

    def get_title(self):
        t = self.relation('bundle')['dernocua']
        return 'Dernocua' if not t else t[0]
    ...
    def generate_archive(self):
        subprocess.check_call('cd %s; tar czf dernocua-%s.tar.gz %s' % (
...
class Note:
    ...
    def author(self):
        return (self.bundle.relation('note-by')[self.notename]
                or self.bundle.relation('by')['dernocua']
                or ['Anonymous'])[0]
